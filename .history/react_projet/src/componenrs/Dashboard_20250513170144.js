...
const TableRow = ({ stagiaire }) => {
  const downloadPDF = () => {
    ...
    const details = [
      ...
      { label: "Nom de l'entreprise :", value: stagiaire.periodes[0]?.stage?.entreprise || 'Non renseigné' },
      { label: "Nom et Prénom Tuteur :", value: `${stagiaire.periodes[0]?.appreciations[0]?.tuteur?.nom || 'Non renseigné'} ${stagiaire.periodes[0]?.appreciations[0]?.tuteur?.prenom || 'Non renseigné'}` },
      { label: "Email de Tuteur :", value: stagiaire.periodes[0]?.appreciations[0]?.tuteur?.email || 'Non renseigné' },
      { label: "Période de stage du :", value: `${stagiaire.periodes[0]?.dateDebut || 'Non renseigné'} au ${stagiaire.periodes[0]?.dateFin || 'Non renseigné'}` },
      {
        label: "THEME DU PROJET PRINCIPAL CONFIE A L'ETUDIANT(E) :", value: stagiaire.periodes[0]?.stage?.description || 'Non renseigné'
      }
    ];
    ...
    const globalAppreciations = stagiaire.periodes[0]?.appreciations[0]?.competences.map((competence) => ({
      label: competence.intitule || 'Non renseigné',
      value: competence.note?.toString() || 'Non renseigné',
    })) || [];

    // Draw table for global appreciations
    let startY = 170;
    const tableColumnWidths = [100, 80];
    const tableHeaders = ["Compétence", "Note"];
    ...
    globalAppreciations.forEach((competence) => {
      doc.rect(20, startY, tableColumnWidths[0], 10);
      doc.rect(120, startY, tableColumnWidths[1], 10);
      doc.text(competence.label, 25, startY + 7);
      doc.text(competence.value, 125, startY + 7);
      startY += 10;
    });
