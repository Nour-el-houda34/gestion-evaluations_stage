...
const TableRow = ({ stagiaire }) => {
  const downloadPDF = () => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    doc.text("Fiche d'Appréciation", 20, 20);
    doc.setFontSize(12);
    doc.text(`Nom: ${stagiaire.nom} ${stagiaire.prenom}`, 20, 40);
    doc.text(`Email: ${stagiaire.email || 'Non renseigné'}`, 20, 50);

    stagiaire.periodes.forEach((periode, index) => {
      const yOffset = 70 + index * 30;
      doc.text(`Période ${index + 1}:`, 20, yOffset);
      doc.text(`  Date Début: ${periode.dateDebut}`, 30, yOffset + 10);
      doc.text(`  Date Fin: ${periode.dateFin}`, 30, yOffset + 20);
      if (periode.appreciations.length > 0 && periode.appreciations[0].tuteur) {
        doc.text(
          `  Tuteur: ${periode.appreciations[0].tuteur.nom} ${periode.appreciations[0].tuteur.prenom}`,
          30,
          yOffset + 30
        );
        doc.text(
          `  Entreprise: ${periode.appreciations[0].tuteur.entreprise}`,
          30,
          yOffset + 40
        );
      } else {
        doc.text("  Tuteur: Non renseigné", 30, yOffset + 30);
        doc.text("  Entreprise: Non renseigné", 30, yOffset + 40);
      }
    });

    doc.save(`Fiche_Appreciation_${stagiaire.nom}_${stagiaire.prenom}.pdf`);
  };
