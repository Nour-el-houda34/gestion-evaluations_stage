...
const Dashboard = () => {
  ...
  const generatePDF = (row) => {
    ...
    // Palette de couleurs
    const colors = {
      primary: [33, 82, 138],      // Bleu foncé
      secondary: [0, 102, 204],    // Bleu vif
      accent: [255, 193, 7],       // Jaune
      text: [33, 37, 41],          // Gris foncé
      tableHead: [224, 242, 254],  // Bleu très clair
      tableBorder: [33, 82, 138],  // Bleu foncé
    };
    ...
    doc.setTextColor(...colors.primary);
    doc.text(`${row.nom} ${row.prenom}`, 55, y);
    doc.setTextColor(...colors.text);
    y += 7;
    doc.text(`Email :`, 20, y);
    doc.setTextColor(...colors.primary);
    doc.text(`${row.email}`, 55, y);
    doc.setTextColor(...colors.text);
    y += 7;
    doc.text(`Entreprise :`, 20, y);
    doc.setTextColor(...colors.primary);
    doc.text(`${stage?.entreprise ?? 'Non renseigné'}`, 55, y);
    doc.setTextColor(...colors.text);
    y += 7;
    doc.text(`Tuteur :`, 20, y);
    doc.setTextColor(...colors.primary);
    doc.text(
      tuteur ? `${tuteur.nom} ${tuteur.prenom}` : "Non renseigné",
      55,
      y
    );
    doc.setTextColor(...colors.text);
    ...
    doc.setTextColor(...colors.primary);
    doc.text(`${row.dateDebut} au ${row.dateFin}`, 55, y);
    y += 10;

    // Thème et objectif du projet principal confié à l'étudiant(e)
    if (stage?.description) {
      doc.setFont("helvetica", "bold").setTextColor(...colors.secondary);
      ...
      y += 6;
      doc.text(themeText, 25, y);
      y += themeText.length * 6 + 2;
    } else if (stage?.theme) {
      doc.setFont("helvetica", "bold").setTextColor(...colors.secondary);
    ...
    if (stage?.objectif) {
      ...
      objectives.forEach((obj, idx) => {
        doc.text(`- ${obj}`, 25, y + 6 + idx * 6);
      });
      y += 6 + objectives.length * 6;
    }
    ...
    if (appreciation?.evaluations?.length > 0) {
      ...
      autoTable(doc, {
        ...
        alternateRowStyles: {
          fillColor: [245, 249, 255], // bleu très très clair
        },
    ...
    if (appreciation?.competences?.length > 0) {
      doc.addPage();
      y = 20;
      doc.setFont("helvetica", "bold").setFontSize(14).setTextColor(...colors.secondary);
      ...
      appreciation.competences.forEach((competence) => {
        if (competence.intitule !== "Compétences spécifiques") {
          ...
          y = doc.lastAutoTable?.finalY ? doc.lastAutoTable.finalY + 8 : y + 28;
        }
      });
      ...
      if (specificCompetence) {
        doc.addPage();
        y = 20;
        doc.setFontSize(13).setFont("helvetica", "bold").setTextColor(...colors.secondary);
