...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  const handleRadioChange = (catIdx, compIdx, value) => {
    ...
    syncChange(updated, competencesSpecifiques);
  };
  ...
  const syncChange = (cats, specs) => {
    handleChange({
      competences: buildCompetencePayload(cats, specs),
      evaluations: buildEvaluationPayload(cats),
    });
  };
