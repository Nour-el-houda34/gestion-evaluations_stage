...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  // Ajout de l'état pour suivre l'index de la catégorie affichée
  const [currentCategoryIdx, setCurrentCategoryIdx] = useState(0);

  // Gestion du passage à la catégorie suivante
  ...
  // Gestion du retour à la catégorie précédente
  ...
  // Gestion du retour global (étape précédente du formulaire principal)
  const handlePrevGlobal = () => {
    setCurrentCategoryIdx(0);
    handlePrev();
  };

  // Gestion de la soumission globale (étape finale)
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl shadow-2xl">
      ...
      >
        {/* ... Modal code inchangé ... */}
        {/* (Gardez le code du Modal tel quel, il n'a pas besoin d'être modifié) */}
        {/* ... */}
      </Modal>

      <div className="space-y-6">
        {/* Affichage d'une seule catégorie à la fois */}
        {categories.length > 0 && currentCategoryIdx < categories.length && (
          <div className="bg-white p-4 rounded-lg shadow-md">
            <h3 className="text-xl font-semibold mb-4 text-blue-700">{categories[currentCategoryIdx].nom}</h3>
            <div className="overflow-x-auto">
              <table className="min-w-full border-collapse border border-gray-300">
                ...
                <tbody>
                  {categories[currentCategoryIdx].competences.map((comp, compIdx) => (
                    <tr key={compIdx} className="even:bg-gray-100">
                      ...
                      {niveaux.map((niveau) => (
                        <td key={niveau} className="border text-center">
                          <input
                            type="radio"
                            name={`cat${currentCategoryIdx}_comp${compIdx}`}
                            value={niveau}
                            checked={categories[currentCategoryIdx].evaluations[compIdx] === niveau}
                            onChange={() => handleRadioChange(currentCategoryIdx, compIdx, niveau)}
            ...
            <div className="mt-4 flex items-center">
              ...
              <input
                ...
                value={categories[currentCategoryIdx].note}
                onChange={(e) => handleNoteChange(currentCategoryIdx, e.target.value)}
              />
            </div>
        ...
        {/* Affichage du formulaire des compétences spécifiques après la dernière catégorie */}
        {currentCategoryIdx === categories.length && (
          <div className="bg-white p-4 rounded-lg shadow-md">
        ...
        {/* Boutons globaux pour revenir à l'étape précédente du formulaire principal */}
        <div className="flex justify-between mt-8">
          <button
            className="px-6 py-3 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 transition-all duration-300 shadow-md"
