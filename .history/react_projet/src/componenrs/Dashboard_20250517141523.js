...
const Dashboard = () => {
  ...
  const generatePDF = (row) => {
    ...
    // Informations générales regroupées (sans le titre)
    ...
    // Titres en gras, valeurs en normal
    doc.setFont("helvetica", "bold").setTextColor(...colors.text);
    doc.text(`Stagiaire :`, 20, y);
    doc.setFont("helvetica", "normal");
    doc.text(`${row.nom} ${row.prenom}`, 50, y);
    y += 7;

    doc.setFont("helvetica", "bold");
    doc.text(`Email :`, 20, y);
    doc.setFont("helvetica", "normal");
    doc.text(`${row.email}`, 50, y);
    y += 7;

    doc.setFont("helvetica", "bold");
    doc.text(`Entreprise :`, 20, y);
    doc.setFont("helvetica", "normal");
    doc.text(`${stage?.entreprise ?? 'Non renseigné'}`, 50, y);
    y += 7;

    doc.setFont("helvetica", "bold");
    doc.text(`Tuteur :`, 20, y);
    doc.setFont("helvetica", "normal");
    doc.text(`${tuteur ? `${tuteur.nom} ${tuteur.prenom}` : 'Non renseigné'}`, 50, y);
    y += 7;

    doc.setFont("helvetica", "bold");
    doc.text(`Date du stage :`, 20, y);
    doc.setFont("helvetica", "normal");
    doc.text(`${row.dateDebut} au ${row.dateFin}`, 50, y);
    y += 10;

    // Thème et objectif du projet principal confié à l'étudiant(e)
    if (stage?.description) {
      doc.setFont("helvetica", "bold").setTextColor(0, 0, 0); // noir
      doc.text("Thème du projet principal confié à l'étudiant(e) :", 20, y);
      doc.setFont("helvetica", "normal").setTextColor(0, 0, 0); // noir
      const themeText = doc.splitTextToSize(stage.description, 160);
      doc.text(themeText, 20, y + 7);
      y += 7 + themeText.length * 6;
    } else if (stage?.theme) {
      doc.setFont("helvetica", "bold").setTextColor(0, 0, 0); // noir
    ...
    if (stage?.objectif) {
      ...
      doc.text(`- ${obj}`, 25, y + 7 + idx * 6);
      });
      y += 7 + objectives.length * 6;
    }

    // Appréciations globales (titre stylé comme "APPRECIATION DU TUTEUR DE STAGE")
    if (appreciation?.evaluations?.length > 0) {
      ...
      autoTable(doc, {
        ...
        startY: y,
        headStyles: {
          fillColor: colors.tableHead, // blue background
    ...
    // Compétences évaluées (titre en noir)
    if (appreciation?.competences?.length > 0) {
      doc.addPage();
      y = 20;
      doc.setFont("helvetica", "bold").setFontSize(16).setTextColor(0, 0, 0); // noir
      ...
      appreciation.competences.forEach((competence) => {
        if (competence.intitule !== "Compétences spécifiques") {
          doc.setFont("helvetica", "bold").setFontSize(12).setTextColor(0, 0, 0); // noir
      ...
      // Compétences spécifiques sur une nouvelle page (titre en noir)
