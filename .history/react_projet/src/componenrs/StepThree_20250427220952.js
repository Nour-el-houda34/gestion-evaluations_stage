...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!stagiaire?.nom || !stage?.entreprise || !tuteur?.nom) {
      setModalMessage("Veuillez remplir toutes les informations nécessaires.");
      setModalType('error');
      setIsModalOpen(true);
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        globalAppreciations.forEach((appreciation) => {
          doc.text(appreciation.label, 25, startY);
          doc.text(appreciation.value, 125, startY);
          startY += 10;
        });

        doc.save("evaluation_stage_etudiant.pdf");
      } else {
  ...
  return (
    <div className="step-three-container">
      {/* Modal Component */}
      <Modal isOpen={isModalOpen} onRequestClose={() => setIsModalOpen(false)} contentLabel="Message">
              ...
              <div>
                {niveaux.map((niveau, idx) => (
                  <label key={idx}>
                    <input
                      type="radio"
                      name={`competence-${catIdx}-${compIdx}`}
                      value={niveau}
