...
const Dashboard = () => {
  ...
  // Génération du PDF avec style de tableau personnalisé (pas autoTable)
  const generatePDF = (row) => {
    ...
    const colors = {
      ...
      tableHead: [226, 232, 240],  // slate-200
    ...
    if (appreciation?.competences?.length > 0) {
      ...
      if (specificCompetence) {
        doc.addPage();
        y = 20;
        doc.setFontSize(13).setFont("helvetica", "bold").setTextColor(...colors.accent);
