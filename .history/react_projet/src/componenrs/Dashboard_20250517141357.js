...
const Dashboard = () => {
  ...
  const generatePDF = (row) => {
    ...
    doc.text(`Stagiaire : ${row.nom} ${row.prenom}`, 20, y);
    y += 7;
    doc.text(`Email : ${row.email}`, 20, y);
    y += 7;
    doc.text(`Entreprise : ${stage?.entreprise ?? 'Non renseigné'}`, 20, y);
    y += 7;
    doc.text(`Tuteur : ${tuteur ? `${tuteur.nom} ${tuteur.prenom}` : 'Non renseigné'}`, 20, y);
    y += 7;
    doc.text(`Date du stage : ${row.dateDebut} au ${row.dateFin}`, 20, y);
    y += 10;
