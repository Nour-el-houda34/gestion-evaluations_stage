...
const Dashboard = () => {
  ...
  const generatePDF = (row) => {
    ...
    const details = [
      ...
      { label: "Période de stage du :", value: `${row.dateDebut} au ${row.dateFin}` },
      {
        ...
        value: stage?.description
          ? doc.splitTextToSize(stage.description, 160).join("\n")
          : "Non renseigné"
      }
    ];
    ...
    details.forEach((detail, index) => {
      ...
      doc.text(detail.label, 20, yDetails);
      ...
      lines.forEach((line, lineIndex) => {
        doc.text(line, 20 + labelWidth + 5, yDetails + lineIndex * 6);
      });
      yDetails += Math.max(10, lines.length * 6);
    });

    // OBJECTIFS
    if (stage?.objectif) {
      doc.setFont("helvetica", "bold").setFontSize(14);
      doc.text("OBJECTIFS ASSIGNES A L'ETUDIANT(E):", 20, yDetails + 5);
      doc.setFont("helvetica", "normal").setFontSize(12);
      const objectives = stage.objectif.split("\n").map((obj) => `- ${obj}`);
      const objLines = doc.splitTextToSize(objectives.join("\n"), 170);
      objLines.forEach((line, idx) => {
        doc.text(line, 25, yDetails + 12 + idx * 6);
      });
      yDetails += 12 + objLines.length * 6;
    }

    // APPRECIATIONS GLOBALES SUR L'ETUDIANT(E)
    doc.setFont("helvetica", "bold").setFontSize(16);
    doc.setTextColor(0, 102, 204);
    doc.text("APPRECIATIONS GLOBALES SUR L'ETUDIANT(E)", 105, yDetails + 20, { align: "center" });
