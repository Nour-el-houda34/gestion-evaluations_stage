...
const FormulaireApreciation = () => {
  ...
  return (
    <div className="max-w-4xl w-full mx-auto p-8 mt-8 bg-white rounded-xl">
      <nav className="mb-1">
        <a
          href="/dashboard"
        ...
        <ol className="flex space-x-12">
          {[1, 2, 3].map((s) => (
            <li key={s} className="flex flex-col items-center">
              <div
                className={`w-12 h-12 flex items-center justify-center rounded-full text-xl font-bold border-2 transition-all duration-200 ${
                  step === s
              ...
              <span className={`mt-3 text-sm font-medium text-center max-w-[200px] ${
      ...
      <section className="p-8 min-h-[350px]  rounded-lg">
        {steps[step]}
      </section>
    </div>
  );
