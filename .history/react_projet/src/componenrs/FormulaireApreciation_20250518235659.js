  ...
  return (
    <div className="max-w-4xl w-full mx-auto p-8 mt-8 bg-white rounded-xl shadow-2xl">
      <nav className="mb-4 flex items-center justify-between">
        ...
        <span title="Formulaire d'appréciation" className="text-blue-500 flex items-center gap-2">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2" fill="#e0f2fe"/>
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 16v-4m0-4h.01" />
          </svg>
          <span className="hidden sm:inline text-sm font-medium">Formulaire d'appréciation</span>
        </span>
      </nav>
          ...
          {[1, 2, 3].map((s) => (
            <li key={s} className="flex flex-col items-center">
              ...
              >
                {stepIcons[s]}
                {step > s && (
                  <span className="absolute -bottom-2 right-0 bg-green-400 rounded-full p-1">
                    {/* Icône de validation */}
