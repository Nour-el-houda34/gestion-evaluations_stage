...
const TableRow = ({ stagiaire }) => {
  const downloadPDF = () => {
    ...
    const details = [
      ...
      { label: "Nom de l'entreprise :", value: stagiaire.periodes[0]?.appreciations[0]?.tuteur?.entreprise || 'Non renseigné' },
    ...
    const globalAppreciations = stagiaire.periodes[0]?.appreciations.map((appreciation) => ({
      label: appreciation.categorie || 'Non renseigné',
      value: appreciation.valeur || 'Non renseigné',
    })) || [];
