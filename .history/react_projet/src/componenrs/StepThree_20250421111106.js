...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  const [isLoaded, setIsLoaded] = useState(false);  // Nouveau state pour vérifier si les données sont déjà chargées

  useEffect(() => {
    // Vérifie si les données sont déjà chargées
    if (!isLoaded && data.appreciation && data.appreciation.competences) {
      ...
      setIsLoaded(true);  // Indique que les données ont été chargées
    }
  }, [data, isLoaded]);  // Le useEffect dépend de 'data' et 'isLoaded'
