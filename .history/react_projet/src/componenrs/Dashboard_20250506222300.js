const TableRow = ({ stagiaire, periode }) => (
  <tr className="border-b hover:bg-gray-50 transition-colors duration-300"></tr>
...
const TableRow = ({ stagiaire }) => (
  <tr className="border-b hover:bg-gray-50 transition-colors duration-300">
    ...
    <td className="px-4 py-3">{stagiaire.institution || 'Non renseigné'}</td>
    {/* Render multiple stages (periodes) for each stagiaire */}
    {stagiaire.periodes.map((periode, index) => (
      <React.Fragment key={index}>
        ...
        {/* You can add more details about the stage here */}
      </React.Fragment>
    ))}
  </tr>
);
        ...
        <table className="w-full table-auto border-collapse">
          <thead>
            <tr className="bg-purple-100 text-purple-800">
              ...
              <th className="px-2 py-1 text-left border">Institution</th>
          ...
          <tbody>
            {stagiaires.map((stagiaire, index) => (
              stagiaire.periodes.map((periode, periodeIndex) => (
                <TableRow key={`${index}-${periodeIndex}`} stagiaire={stagiaire} />
              ))
            ))}
          </tbody>
        </table>
