...
const Dashboard = () => {
 ...
 useEffect(() => {
  // Récupérer le token depuis le stockage local, session, contexte ou autre
  const token = localStorage.getItem('token');  
  console.log('Token récupéré:', token);  // Pour vérifier
  if (!token) {
    console.warn("Pas de token trouvé, rediriger vers login ?");
    return;
  }
