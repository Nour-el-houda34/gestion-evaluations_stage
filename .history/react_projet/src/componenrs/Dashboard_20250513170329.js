...
const TableRow = ({ stagiaire }) => {
  const downloadPDF = () => {
    ...
    // Récupération de la période et de l’appréciation associée
    ...
    const details = [
      { label: "Nom et Prénom de Stagiaire :", value: `${stagiaire.nom || 'Non renseigné'} ${stagiaire.prenom || ''}` },
      ...
      { label: "Nom et Prénom Tuteur :", value: `${tuteur.nom || 'Non renseigné'} ${tuteur.prenom || ''}` },
    ...
    // Affichage des détails avec gestion du wrapping
    let currentY = 55;
    details.forEach((detail) => {
      ...
      lines.forEach((line, lineIndex) => {
        doc.text(line, 25 + labelWidth, currentY + lineIndex * 6);
      });
  
      currentY += lines.length * 6 + 4;
    });
  
    // Titre section appréciations globales
    doc.setFont("helvetica", "bold").setFontSize(16);
    doc.setTextColor(0, 102, 204);
    doc.text("APPRECIATIONS GLOBALES SUR L'ETUDIANT(E)", 105, currentY + 10, { align: "center" });
  
    // Compétences / notes
    const globalAppreciations = appreciation.competences?.map((competence) => ({
      label: competence.intitule || 'Non renseigné',
      value: competence.note?.toString() || 'Non renseigné',
    })) || [];
  
    // Affichage tableau compétences
    ...
    // Sauvegarde
    const nomFichier = `evaluation_${stagiaire.nom?.replace(/\s+/g, '_')}_${stagiaire.prenom?.replace(/\s+/g, '_')}.pdf`;
    doc.save(nomFichier);
  };
