...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  // Ajout de l'état pour la navigation entre catégories
              ...
              {modalType === "success" && (
                <button
                  onClick={() => {
                    ...
                    // Raccourci pour autoTable
                    const autoTable = window.jspdfAutoTable || (doc.autoTable && doc.autoTable.bind(doc));
                    // Titre principal
                    ...
                    const infoRows = [
                      ...
                      { label: "Tuteur", value: payload.tuteur ? `${payload.tuteur.nom} ${payload.tuteur.prenom}` : 'Non renseigné' },
                    ...
                    if (payload.stage?.theme || payload.stage?.description) {
                      doc.setFont("helvetica", "bold").setTextColor(0, 0, 0);
                      doc.text("Thème :", 20, y);
                      doc.setFont("helvetica", "normal").setTextColor(0, 0, 0);
                      const themeLine = `${payload.stage?.theme ?? ""}${payload.stage?.theme && payload.stage?.description ? " - " : ""}${payload.stage?.description ?? ""}`;
                    ...
                    if (payload.appreciation?.evaluations?.length > 0 && autoTable) {
                      y += 5;
                    ...
                    if (payload.appreciation?.competences?.length > 0 && autoTable) {
                      doc.addPage();
                      y = 20;
                      doc.setFontSize(16);
                      doc.setTextColor(...colors.subtitle);
