...
// Récupère les compétences spécifiques si elles existent
const getInitialSpecifics = (data) => {
...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  const [categories, setCategories] = useState(() => getInitialCategories(data));
  const [competencesSpecifiques, setCompetencesSpecifiques] = useState(() =>
    getInitialSpecifics(data)
  );
