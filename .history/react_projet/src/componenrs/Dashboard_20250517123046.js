...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8081/api/statistiques/stages-par-filiere')
      .then(res => setStagesParFiliere(res.data));

    axios.get('http://localhost:8081/api/statistiques/stages-par-entreprise')
      .then(res => setStagesParEntreprise(res.data));

    axios.get('http://localhost:8081/api/statistiques/evolution-stages')
      .then(res => setEvolutionStages(res.data));
  }, []);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 space-y-10">
      ...
      <section className="bg-white p-6 rounded-xl shadow">
        <h3 className="text-xl font-bold mb-4 text-indigo-800">🎓 Stagiaires par Filière</h3>
        <Bar
          data={{
            labels: stagesParFiliere.map(item => item.filiere),
            datasets: [{
              label: 'Nombre de stagiaires',
              data: stagesParFiliere.map(item => item.count),
      ...
      <section className="bg-white p-6 rounded-xl shadow">
        <h3 className="text-xl font-bold mb-4 text-indigo-800">📅 Évolution des Stages</h3>
        <Line
          data={{
            labels: evolutionStages.map(item => item.periode),
            datasets: [{
              label: 'Stages par mois',
              data: evolutionStages.map(item => item.count),
              fill: false,
              borderColor: '#6366F1',
              tension: 0.3,
            }],
