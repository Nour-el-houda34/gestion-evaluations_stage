...
return (
  <div className="min-h-screen bg-gradient-to-br from-gray-100 to-indigo-50 p-6 space-y-10 max-w-7xl mx-auto">
    ...
    <section className="grid grid-cols-1 sm:grid-cols-3 gap-6">
      {[
        ...
        <div
          key={i}
          className={`bg-gradient-to-br ${bgFrom} ${bgTo} p-7 rounded-2xl shadow-lg text-center text-white flex flex-col items-center transition-transform transform hover:scale-105 border-b-4 border-white/30`}
          style={{ minWidth: '170px' }}
        >
          <div className="mb-2 text-4xl drop-shadow">{icon}</div>
    ...
    {/* Section unique avec Bar chart à gauche et Table à droite */}
    <section className="bg-white/80 backdrop-blur p-7 rounded-3xl shadow-xl mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 border border-indigo-100">
      {/* Bar chart à gauche */}
      <div className="flex flex-col justify-between h-full">
        <h3 className="text-xl font-semibold mb-4 text-indigo-700 flex items-center gap-2 select-none">
      ...
      {/* Table des stagiaires à droite */}
      <div className="flex flex-col h-full">
        <h3 className="text-xl font-semibold mb-4 text-indigo-700 select-none flex items-center gap-2">
          <span className="text-2xl">📋</span> Liste des Stagiaires
        </h3>
        <input
          type="text"
          placeholder="Rechercher un stagiaire..."
          className="mb-4 p-2 border border-gray-300 rounded-md w-full focus:outline-none focus:ring-2 focus:ring-indigo-400 transition text-sm shadow-sm"
          value={searchTerm}
