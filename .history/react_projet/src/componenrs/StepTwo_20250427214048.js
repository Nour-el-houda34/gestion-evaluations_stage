...
const StepTwo = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  // Mise à jour des valeurs des boutons radio en fonction des données lors du rendu
  useEffect(() => {
    if (data.appreciation && data.appreciation.evaluations) {
      // Réinitialisation de l'état avec les évaluations actuelles de `data.appreciation.evaluations`
  ...
  }, [data]); // Rechargement des données à chaque fois que `data` change.

  // Fonction pour changer les valeurs des radios
  const handleRadioChange = (setter) => (e) => setter(e.target.value);

  const handleNext = () => {
    // Mise à jour des évaluations dans `data`
    ...
    // Appel de `handleChange` pour mettre à jour les données dans le parent
    ...
    <div className="max-w-3xl mx-auto p-6 bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl shadow-2xl">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white p-4 rounded-lg shadow-md">
          <label className="block text-lg font-semibold text-gray-700">Implication dans ses activités :</label>
        ...
        {/* Ouverture */}
        <div className="bg-white p-4 rounded-lg shadow-md">
          <label className="block text-lg font-semibold text-gray-700">Ouverture aux autres :</label>
          <div className="flex flex-wrap gap-4 mt-2">
        ...
        <div className="bg-white p-4 rounded-lg shadow-md">
          <label className="block text-lg font-semibold text-gray-700">Qualité de ses productions :</label>
          <div className="flex flex-wrap gap-4 mt-2">
        ...
        {/* Observations */}
        <div className="bg-white p-4 rounded-lg shadow-md">
          <label className="block text-lg font-semibold text-gray-700">Observations sur l'ensemble du travail accompli :</label>
          <textarea
            name="observations"
            value={observations}
