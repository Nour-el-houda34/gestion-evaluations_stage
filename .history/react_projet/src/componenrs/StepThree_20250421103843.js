...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  const isFirstRender = useRef(true); // ref pour éviter l'appel au premier render

  useEffect(() => {
    if (isFirstRender.current) {
      isFirstRender.current = false;
      return; // On ignore le premier render
    }
  ...
  }, [categories, competencesSpecifiques, data.appreciation, handleChange]);
