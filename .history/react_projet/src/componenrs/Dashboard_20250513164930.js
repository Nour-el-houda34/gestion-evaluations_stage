...
const TableRow = ({ stagiaire }) => {
  const downloadPDF = () => {
    ...
    const details = [
      ...
      { label: "Nom et Prénom Tuteur :", value: `${stagiaire.periodes[0]?.appreciations[0]?.tuteur?.nom || ''} ${stagiaire.periodes[0]?.appreciations[0]?.tuteur?.prenom || ''}` },
      ...
      { label: "Période de stage du :", value: `${stagiaire.periodes[0]?.dateDebut || 'Non renseigné'} au ${stagiaire.periodes[0]?.dateFin || 'Non renseigné'}` },
      {
        ...
        value: doc.splitTextToSize(stagiaire.periodes[0]?.appreciations[0]?.description || 'Non renseigné', 160).join("\n")
      }
    ];
