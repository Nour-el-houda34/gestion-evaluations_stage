    ...
    try {
      ...
      if (response.ok) {
        ...
        payload.appreciation.evaluations.forEach((evaluation, index) => {
          ...
          doc.text(`${evaluation.categorie}:`, 20, evalStartY, { align: "left" });

          doc.setFont("helvetica", "normal").setFontSize(12).setTextColor(0, 0, 0);
          doc.text(`${evaluation.valeur}`, 120, evalStartY, { align: "left" });

          evalStartY += 10; // Move to the next line after each category-value pair
        });
