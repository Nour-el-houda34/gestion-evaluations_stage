  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Set title fonts and size for the center-aligned titles
        ...
        // Stagiaire and Stage Information with enhanced titles
        doc.setTextColor(0, 102, 204);  // Blue color for titles
        doc.text("Nom et Prénom de Stagiaire :", 20, 45);
        doc.setTextColor(0, 0, 0); // Reset color to black
        doc.text(`${payload.stagiaire.nom} ${payload.stagiaire.prenom}`, 60, 45);
  
        doc.setTextColor(0, 102, 204);
        doc.text("Nom de l'entreprise :", 20, 55);
        doc.setTextColor(0, 0, 0);
        doc.text(payload.stage.entreprise, 60, 55);
  
        doc.setTextColor(0, 102, 204);
        doc.text("Nom et Prénom Tuteur :", 20, 65);
        doc.setTextColor(0, 0, 0);
        doc.text(`${payload.tuteur.nom} ${payload.tuteur.prenom}`, 60, 65);
  
        doc.setTextColor(0, 102, 204);
        doc.text("Période de stage du :", 20, 75);
        doc.setTextColor(0, 0, 0);
        doc.text(`${payload.periode.dateDebut} au ${payload.periode.dateFin}`, 60, 75);
  
        doc.setTextColor(0, 102, 204);
        doc.text("THEME DU PROJET PRINCIPAL CONFIE A L'ETUDIANT(E) :", 20, 85);
        doc.setTextColor(0, 0, 0);
        doc.text(payload.stage.description, 60, 85);
  
        doc.setTextColor(0, 102, 204);
        doc.text("OBJECTIFS ASSIGNES A L'ETUDIANT(E):", 20, 95);
        doc.setTextColor(0, 0, 0);
        doc.text(payload.stage.objectif, 60, 95);
  
        // Add another separator line
        doc.line(20, 105, 195, 105);
  
        // Evaluation section with improved title style
        doc.setTextColor(0, 102, 204);
        ...
        doc.text("NA : Non applicable - La compétence n'a pas été mise en œuvre, ou très peu.", 20, 140);
        doc.text("Débutant : Applique les connaissances avec assistance.", 20, 150);
        doc.text("Autonome : Met en œuvre les pratiques de manière indépendante.", 20, 160);
        doc.text("Autonome + : Résout des problèmes en fonction du contexte professionnel -", 20, 170);
        ...
        payload.appreciation.competences.forEach((competence) => {
          ...
          // Table headers for subcategories with background color
          doc.setFontSize(10);
          doc.setFont("helvetica", "normal");
          doc.setTextColor(255, 255, 255);  // White text for table headers
          doc.setFillColor(0, 102, 204);  // Blue background for headers
          doc.rect(30, yPosition - 5, 100, 10, "F"); // Background rectangle
          doc.text("Sous-catégories", 30, yPosition);
          
          doc.setFillColor(0, 102, 204);  // Blue background for the "Note" column
