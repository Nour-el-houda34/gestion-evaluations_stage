...
const Dashboard = () => {
  ...
  const generatePDF = (row) => {
    ...
    // Palette de couleurs compatibles
    const colors = {
      primary: [33, 150, 243],      // bleu
      secondary: [76, 175, 80],     // vert
      accent: [255, 193, 7],        // jaune
      text: [33, 37, 41],           // gris foncé
      tableHead: [236, 239, 241],   // gris clair
      tableBorder: [189, 189, 189], // gris moyen
    };
    ...
    doc.text("APPRECIATION DU TUTEUR DE STAGE", 105, 35, { align: "center" });
    ...
    doc.text(`Email: ${row.email}`, 20, y + 8);
    doc.text(`Entreprise: ${stage?.entreprise ?? 'Non renseigné'}`, 20, y + 16);
    doc.text(`Tuteur: ${tuteur ? `${tuteur.nom} ${tuteur.prenom}` : 'Non renseigné'}`, 20, y + 24);
    doc.text(`Date du stage: ${row.dateDebut} au ${row.dateFin}`, 20, y + 32);
    y += 40;
    ...
    if (appreciation?.competences?.length > 0) {
      ...
      appreciation.competences.forEach((competence) => {
        if (competence.intitule !== "Compétences spécifiques") {
          doc.setFont("helvetica", "bold").setFontSize(12).setTextColor(...colors.secondary);
      ...
      if (specificCompetence) {
        doc.addPage();
        y = 20;
        doc.setFontSize(14).setFont("helvetica", "bold").setTextColor(...colors.accent);
