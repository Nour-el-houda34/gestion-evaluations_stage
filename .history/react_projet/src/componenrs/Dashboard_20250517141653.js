...
const Dashboard = () => {
  ...
  const generatePDF = (row) => {
    ...
    // Informations générales organisées et harmonisées (chaque info sur une seule ligne)
    const infoRows = [
    ...
    const labelWidth = 38; // Largeur pour aligner les valeurs
    infoRows.forEach(({ label, value }) => {
      doc.setFont("helvetica", "bold").setTextColor(...colors.text);
      doc.text(`${label} :`, 20, y, { baseline: 'top' });
      doc.setFont("helvetica", "normal").setTextColor(...colors.text);
      doc.text(value, 20 + labelWidth, y, { baseline: 'top' });
      y += 7;
    });

    // Thème du projet principal
    if (stage?.description || stage?.theme) {
      doc.setFont("helvetica", "bold").setTextColor(0, 0, 0);
      doc.text(
      stage?.description
        ? "Thème du projet principal confié à l'étudiant(e) :"
        : "Thème du projet :",
      20,
      y
      );
      doc.setFont("helvetica", "normal").setTextColor(0, 0, 0);
      const themeText = doc.splitTextToSize(
      stage?.description || stage?.theme,
      160
      );
    ...
    if (stage?.objectif) {
      ...
      doc.splitTextToSize(`- ${obj}`, 160)
      );
      doc.text(objLines, 25, y + 7);
