  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        doc.text(`Nom et Prénom de Stagiaire : ${payload.stagiaire.nom} ${payload.stagiaire.prenom}`, 20, 45);
        doc.text(`Nom de l'entreprise : ${payload.stage.entreprise}`, 20, 55);
        doc.text(`Nom et Prénom Tuteur : ${payload.tuteur.nom} ${payload.tuteur.prenom}`, 20, 65);
        doc.text(`Période de stage du : ${payload.periode.dateDebut} au ${payload.periode.dateFin}`, 20, 75);
        
        // Stage project details
        doc.text(`THEME DU PROJET PRINCIPAL CONFIE A L'ETUDIANT(E) : ${payload.stage.description}`, 20, 85);
        doc.text(`OBJECTIFS ASSIGNES A L'ETUDIANT(E): ${payload.stage.objectif}`, 20, 95);
