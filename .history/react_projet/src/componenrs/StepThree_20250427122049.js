  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const doc = new jsPDF();
        doc.setFontSize(18);
        doc.text("Évaluation Stage Etudiants", 20, 20);
        doc.text("APPRECIATION DU TUTEUR DE STAGE", 20, 30);
  
        doc.setFontSize(12);
        doc.text(`Nom et Prénom de Stagiaire : ${payload.stagiaire.nom} ${payload.stagiaire.prenom}`, 20, 40);
        doc.text(`Nom de l'entreprise : ${payload.stage.entreprise}`, 20, 50);
        doc.text(`Nom et Prénom Tuteur : ${payload.tuteur.nom} ${payload.tuteur.prenom}`, 20, 60);
        doc.text(`Période de stage du : ${payload.periode.dateDebut} au ${payload.periode.dateFin}`, 20, 70);
  
        doc.text(`THEME DU PROJET PRINCIPAL CONFIE A L'ETUDIANT(E) : ${payload.stage.description}`, 20, 80);
        doc.text(`OBJECTIFS ASSIGNES A L'ETUDIANT(E): ${payload.stage.objectif}`, 20, 90);
        ...
        // Setting up a table style for competencies
        let startY = 180;
        let rowHeight = 10;
        let tableColumnWidth = [100, 50, 30];
        let yPosition = startY;
  
        // Iterate over the competencies to generate a structured table-like layout
        payload.appreciation.competences.forEach((competence) => {
          ...
          competence.categories.forEach((subcat) => {
            ...
            doc.text(subcat.valeur.toString(), 150, yPosition); // Assuming "valeur" is the score or evaluation result
            yPosition += rowHeight;
