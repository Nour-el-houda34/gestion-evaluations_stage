...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  // Synchronisation avec data.appreciation.competences quand ça change
  useEffect(() => {
    const restored = initCategories.map((cat) => {
      const saved = data.appreciation?.competences?.find(
        (c) => c.intitule === cat.nom
      );
      return {
        ...cat,
        note: saved?.note?.toString() || "",
        evaluations: cat.competences.map((comp) => {
          const found = saved?.categories?.find((sc) => sc.intitule === comp);
          return found?.valeur || "";
        }),
      };
    });
  ...
  const handleRadioChange = (catIdx, compIdx, value) => {
    ...
    handleChange({ competences: buildCompetencePayload(updated, competencesSpecifiques) });
  };
  ...
  const handleSpecificChange = (idx, key, value) => {
    ...
    handleChange({ competences: buildCompetencePayload(categories, updated) });
  };
            ...
            <table className="min-w-full border-collapse border border-gray-300">
              <thead className="bg-indigo-100">
                <tr>
                  ...
                  {niveaux.map((niveau) => (
                    <th key={niveau} className="border border-gray-300 p-2 text-center">{niveau}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
