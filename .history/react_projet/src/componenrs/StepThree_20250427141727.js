  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const doc = new jsPDF();
        doc.setFont("helvetica", "bold").setFontSize(20).setTextColor(0, 51, 102);
        ...
        doc.setFontSize(12).setFont("helvetica", "normal").setTextColor(0, 0, 0);
        ...
        details.forEach((detail, index) => {
          ...
          doc.text(detail.value, 80, 50 + index * 10);
        });

        doc.setFont("helvetica", "bold").setFontSize(16).setTextColor(0, 102, 204);
        ...
        payload.appreciation.evaluations.forEach((evaluation, index) => {
          doc.text(`${evaluation.categorie}: ${evaluation.valeur}`, 20, evalStartY + index * 10);
        });

        evalStartY += (payload.appreciation.evaluations.length * 10) + 10;

        doc.setFont("helvetica", "bold").setFontSize(16).setTextColor(0, 102, 204);
        doc.text("EVALUATIONS DES COMPETENCES DE L'ETUDIANT(E)", 105, evalStartY, { align: "center" });

        let startY = evalStartY + 10;
        const rowHeight = 10;
