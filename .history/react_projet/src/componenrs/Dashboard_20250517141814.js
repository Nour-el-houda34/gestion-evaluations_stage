...
const Dashboard = () => {
  ...
  const generatePDF = (row) => {
    ...
    // Informations générales organisées et harmonisées
    const infoRows = [
    ...
    if (stage?.description || stage?.theme) { doc.setFont("helvetica", "bold").setTextColor(0, 0, 0); doc.text(stage?.description ? "Thème du projet principal confié à l'étudiant(e) :" : "Thème du projet :", 20, y); doc.setFont("helvetica", "normal").setTextColor(0, 0, 0); const themeText = doc.splitTextToSize(stage?.description || stage?.theme, 160); doc.text(themeText, 20, y + 7); y += 7 + themeText.length * 6; }
