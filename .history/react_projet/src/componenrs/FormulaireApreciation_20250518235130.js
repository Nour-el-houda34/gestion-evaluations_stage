...
const FormulaireApreciation = () => {
  ...
  return (
    <div className="max-w-4xl w-full mx-auto p-8 mt-8 bg-white rounded-xl shadow-2xl">
          ...
          {[1, 2, 3].map((s) => (
            <li key={s} className="flex flex-col items-center">
              ...
              <span className={`mt-3 text-sm font-medium text-center max-w-[250px] ${
                ...
                {s === 1 && (
                  <>
                    <FaUserTie className="inline mr-1 text-blue-500" /> Appréciation du Tuteur de Stage
                  </>
                )}
                {s === 2 && (
                  <>
                    <FaRegSmile className="inline mr-1 text-yellow-500" /> Appréciations globales sur l'étudiant(e)
                  </>
                )}
                {s === 3 && (
                  <>
                    <FaTasks className="inline mr-1 text-green-600" /> Évaluations des Compétences
                  </>
                )}
              </span>
            </li>
      ...
      <section className="p-8 min-h-[350px] rounded-lg bg-gray-50 shadow-inner">
        {steps[step]}
      </section>
    </div>
  );
