...
doc.text("Pour chaque compétence évaluée, les niveaux possibles sont les suivants :", 20, 130);  
  ...
  doc.text(" Met en œuvre les pratiques de manière indépendante.", 60, 160);  
    ...
    yPosition += rowHeight + 5;  // Modified for more space below title  
    ...
    yPosition += rowHeight + 5; // Adjust this line to create more space  
    doc.setFont("helvetica", "normal");  
    doc.setTextColor(0, 0, 0);   
    ...
    competence.categories.forEach((subcat) => {  
        ...
        // Adjust the X-position if necessary to avoid overlap  
        const evaluationXPosition = 150;  
        doc.text(subcat.valeur.toString(), evaluationXPosition, yPosition);  
        
        yPosition += rowHeight + 5; // Add additional space between rows (adjust as needed)   
