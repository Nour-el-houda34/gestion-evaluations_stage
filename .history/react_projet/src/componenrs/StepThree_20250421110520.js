...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  useEffect(() => {
    if (data?.appreciation?.competences && categories.length === 0) {
      // Réinitialiser les catégories uniquement si elles sont vides
  ...
  }, [data]); // Écoute uniquement les changements de "data"
