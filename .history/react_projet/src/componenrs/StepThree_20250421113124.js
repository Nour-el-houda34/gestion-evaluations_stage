...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  useEffect(() => {
    if (categories.length === 0 && data?.appreciation?.competences?.length) {
      const restored = data.appreciation.competences
        .filter((comp) => comp.intitule !== "Compétences spécifiques")
        .map((savedCat) => {
          const staticCat = initCategories.find((c) => c.nom === savedCat.intitule);
  
          const competences = staticCat?.competences || savedCat.categories.map(c => c.intitule);
  
          return {
            nom: savedCat.intitule,
            competences: competences,
            note: savedCat.note?.toString() || "",
            evaluations: competences.map((compName) => {
              const found = savedCat.categories.find((c) => c.intitule === compName);
              return found?.valeur || "";
            }),
          };
        });
