  ...
  const generatePDF = (row) => {
    ...
    if (appreciation?.competences?.length > 0) {
      ...
      appreciation.competences.forEach((comp) => {
        doc.setFont("helvetica", "bold").text(`${comp.intitule} (Note: ${comp.note}/20)`, 20, y);
        y += 5;
        ...
        y = result?.finalY ? result.finalY + 10 : y + 30;
      });
    }
