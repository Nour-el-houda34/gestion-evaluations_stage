...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      doc.setFontSize(18);
      doc.text("Fiche d'évaluation de stage", 20, 20);

      doc.setFontSize(12);
      doc.text(`Stagiaire : ${payload.stagiaire.nom} ${payload.stagiaire.prenom}`, 20, 40);
      doc.text(`Institution : ${payload.stagiaire.institution}`, 20, 50);

      doc.text(`Entreprise : ${payload.stage.entreprise}`, 20, 70);
      doc.text(`Objectif du stage : ${payload.stage.objectif}`, 20, 80);

      doc.text(`Tuteur : ${payload.tuteur.nom} ${payload.tuteur.prenom}`, 20, 100);

      doc.text(`Période : ${payload.periode.dateDebut} au ${payload.periode.dateFin}`, 20, 120);

      // Un petit espace pour les compétences
      doc.text("Compétences évaluées :", 20, 140);

      let y = 150;
      payload.appreciation.competences.forEach((cat) => {
        doc.text(`- ${cat.intitule} (Note: ${cat.note}/20)`, 20, y);
        y += 10;
        cat.categories.forEach((subcat) => {
          doc.text(`   • ${subcat.intitule} : ${subcat.valeur}`, 30, y);
          y += 8;
          if (y > 280) { // Si on dépasse la page
            doc.addPage();
            y = 20;
          }
        });
