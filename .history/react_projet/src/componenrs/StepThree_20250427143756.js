...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      globalAppreciations.forEach((appreciation, index) => {
        doc.setFont("helvetica", "bold").setFontSize(14);
        doc.setTextColor(0, 0, 0);
        let labelWidth = doc.getTextWidth(appreciation.label + ":");
        doc.text(`${appreciation.label}:`, 20, 135 + index * 10);
        doc.setFont("helvetica", "normal").setFontSize(12);
        doc.text(appreciation.value, 20 + labelWidth + 5, 135 + index * 10);
      });
