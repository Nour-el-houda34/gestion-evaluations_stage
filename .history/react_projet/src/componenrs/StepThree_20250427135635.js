...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        details.forEach((detail, index) => {  
          ...
          // Couleur de texte normale (ex. : bleu)  
          doc.setTextColor(0, 0, 0); // RGB pour le bleu  
          doc.text(detail.value, 20 + labelWidth + 5, 45 + index * 10); // Marge de 5 pixels entre le label et la valeur  
      });     
      ...
      doc.line(20, 105, 195, 105);
        ...
        doc.setTextColor(0, 0, 139); // RGB pour un bleu sombre (Navy) 
        doc.text("APPRECIATIONS GLOBALES SUR L'ETUDIANT(E)",105, 120, { align: "center" });
        doc.setTextColor(0, 0, 0);
        doc.setFontSize(12);  
        doc.setFont("helvetica", "normal");
        doc.text("IMPLICATION DANS SES ACTIVITES", 20, 130);  

        
        // ... Code d'initialisation ...  

        // Titre  
        doc.text("ÉVALUATION DES ACTIVITÉS", 20, 130);  

        // Position de départ pour les évaluations  
        let evalStartY = 140; // Position initiale pour les évaluations  
        doc.setFontSize(12);  
        doc.setFont("helvetica", "normal");  

        payload.appreciation.evaluations.forEach((evaluation, index) => {  
            // Affichage des détails d'évaluation  
            doc.text(`${evaluation.intitule}: ${evaluation.note}`, 20, evalStartY + index * 10);  
        });  

        // Ajout d'un petit espace après les évaluations  
        evalStartY += (payload.appreciation.evaluations.length * 10) + 10; // Espace total pour les évaluations  

        // Ici, vous pouvez continuer avec d'autres sections de votre PDF  
        ...
        doc.setFont("helvetica", "bold").setFontSize(16);
        doc.setTextColor(0, 0, 139); // RGB pour un bleu sombre (Navy) 
        doc.text("EVALUATIONS DES COMPETENCES DE L'ETUDIANT(E)",105, 200, { align: "center" });
        doc.setTextColor(0, 0, 0);
        // Evaluation details
      // Texte normal  
    doc.setFontSize(12);  
    ...
    doc.text("Pour chaque compétence évaluée, les niveaux possibles sont les suivants :", 20, 130);  
          ...
          doc.setFont("helvetica", "bold").setFontSize(14);
          doc.text("NA :", 20, 220);  
          doc.setFont("helvetica", "normal"); // Retourne à la police normale  
          doc.text(" Non applicable - La compétence n'a pas été mise en œuvre, ou très peu.", 60, 140);  
            doc.setFont("helvetica", "bold").setFontSize(14); // Met la police en gras  
          doc.text("Débutant :", 20, 230);  
          doc.setFont("helvetica", "normal"); // Retourne à la police normale  
          doc.text(" Applique les connaissances avec assistance.", 60, 150);  

        doc.setFont("helvetica", "bold").setFontSize(14); // Met la police en gras  
      doc.text("Débutant :", 20, 240);  
      doc.setFont("helvetica", "normal"); // Retourne à la police normale  
      doc.text(" Met en œuvre les pratiques de manière indépendante.", 60, 160);  

          doc.setFont("helvetica", "bold").setFontSize(14); // Met la police en gras  
        doc.text("Autonome + :", 20, 250);  
        doc.setFont("helvetica", "normal"); // Retourne à la police normale  
        doc.text(" Résout des problèmes en fonction du contexte professionnel -", 60, 260);  
        // Start table for competencies
        let startY = 280;
        let rowHeight = 10;
        let tableColumnWidth = [100, 50, 30];
        let yPosition = startY;
  
        // Loop through competencies and display them in a table layout
        payload.appreciation.competences.forEach((competence) => {  
          // Compétence title and score  
          doc.setFontSize(14);  
          doc.setFont("helvetica", "bold");  
          
          // Vérifiez si la compétence est une catégorie spécifique  
          if (competence.intitule !== "Compétences spécifiques") {  
              doc.text(`${competence.intitule} (Note: ${competence.note}/20)`, 20, yPosition);  
          } else {  
              doc.text(`${competence.intitule}`, 20, yPosition); // Affiche juste le titre  
          }  
          ...
          yPosition += rowHeight; // Adjust this line to create more space  
          doc.setFont("helvetica", "normal");  
          doc.setTextColor(0, 0, 0);   
          ...
          competence.categories.forEach((subcat) => {  
              ...
              // Add some space to the right for better readability  
              const evaluationXPosition = 150; // Adjust the X-position if necessary  
              doc.text(subcat.valeur.toString(), evaluationXPosition, yPosition);  
