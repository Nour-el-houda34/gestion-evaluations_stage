...
const StepThree = ({ nextStep, prevStep, handleChange, data }) => {
  ...
  const handlePrev = () => {
    // Enregistrer les modifications actuelles dans les données
    const updatedCompetences = buildCompetencePayload(categories, competencesSpecifiques);
    const updatedData = {
      ...data,
      appreciation: {
        ...data.appreciation,
        competences: updatedCompetences, // Met à jour les compétences dans les données de l'étape actuelle
      }
    };
  
    // Appeler handleChange pour transmettre les données mises à jour à l'étape précédente
    handleChange(updatedData);  // handleChange est supposé être une fonction parent pour gérer les modifications de données
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload = {
        ...
        appreciation: {
          evaluations: data.appreciation.evaluations, // ✅ garde les vraies valeurs de StepTwo
          competences: buildCompetencePayload(categories, competencesSpecifiques), // ✅ valeurs de StepThree
        }
